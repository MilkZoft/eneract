...
function sendResponse($response) {
	...
	if ($CKEditorFuncNum == -1) {
        echo  "<script type=\"text/javascript\">console.log('something went totally wrong')</script>";
	} elseif ($response[0] === true and $CKEditorFuncNum != -1) {
...
function upload() {
	...
	if (!empty($_GET) and isset($_GET['type']) and array_key_exists($_GET['type'], $config['ResourceType'])) {
		$rType = $config['ResourceType'][$_GET['type']];
	} else {
		return array(false, 'wrong request');
    }

	if (!empty($_FILES) and $_FILES['upload']['size'] > 0 and ($_FILES['upload']['size'] < $rType['maxSize'] or $rType['maxSize'] == 0)) {
    ...
    if ($_GET['type'] == 'Images' and isset($_GET['resize'])) {
	...
	if ($_GET['type'] == 'Images' and isset($_GET['makeThumb'])) {		
...
function resizeImg($sourceFile, $width = 200, $height = 140, $quality = 90, $resizeself = false, $resizeOnLess = true) {
    ...
    if (isset($ermsg) or false === $oImage) {
        return false;
    }
	...
	if (!$resizeOnLess and $xscale < 1 and $yscale < 1) {
        return true;
	}
